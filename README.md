...
- **🔄 自动化无感重建**：通过全局自动重建策略，只需配置一次，所有运行中的服务器及新创建的服务器将自动继承该保护规则。当流量触及阈值，系统自动根据选定快照无缝重建机器并保留原IP。
- **📱 跨平台一键管控**：提供现代化的无缝 Cookie 认证面板，同时支持关联 Telegram Bot，在手机端通过 `/upgrade` 等指令轻松运维。
- **📸 快照与计划任务管理**：支持直接通过界面管理、预估与创建机器快照，并在“删除服务器”时支持设定未来某一具体时间进行 **定时计划删除**（支持一键根据机器账期周年日或月底快速预填时间）。
- **🛡️ 强大的防呆保护**：对于未设定全局策略的服务器，引入精准防侧漏设计：达到限额前 1TB 持续警告，触碰限额立即自动关机，根除流量溢出账单隐患。
